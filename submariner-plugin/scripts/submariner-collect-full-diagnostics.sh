...
if [ -n "$KUBECONFIG2" ] && [ -n "$CONTEXT1" ] && [ -n "$CONTEXT2" ]; then
    ...
    if [ "$SKIP_VERIFY" = "false" ]; then
        ...
        echo "  Start time: $(date '+%Y-%m-%d %H:%M:%S')"
        VERIFY_CMD="KUBECONFIG=${MERGED_KUBECONFIG} subctl verify --context ${CONTEXT1} --tocontext ${CONTEXT2} --only ${VERIFY_CONNECTIVITY_FLAG} --verbose ${IMAGE_OVERRIDE}"
        echo "========================================" > "${OUTPUT_DIR}/verify/connectivity.txt"
        echo "Command executed:" >> "${OUTPUT_DIR}/verify/connectivity.txt"
        echo "${VERIFY_CMD}" >> "${OUTPUT_DIR}/verify/connectivity.txt"
        echo "========================================" >> "${OUTPUT_DIR}/verify/connectivity.txt"
        echo "" >> "${OUTPUT_DIR}/verify/connectivity.txt"
        KUBECONFIG="${MERGED_KUBECONFIG}" subctl verify --context "${CONTEXT1}" --tocontext "${CONTEXT2}" --only "${VERIFY_CONNECTIVITY_FLAG}" --verbose ${IMAGE_OVERRIDE} >> "${OUTPUT_DIR}/verify/connectivity.txt" 2>&1 || echo "Connectivity verification failed or timed out" >> "${OUTPUT_DIR}/verify/connectivity.txt"
